...
type
  TFrmConsultaTicket = class(TFrmConsultaBase)
    ...
    procedure rectIncluirClick(Sender: TObject);
  private
...
begin
  ...
  if not Assigned(FrmCadastroTicket) then
    FrmCadastroTicket := TFrmCadastroTicket.Create(Self, FrameTicket.Ticket.Id, FrameTicket.Ticket.Funcionario);
  FrmCadastroTicket.OnExecutarAposInserirOuAtualizar := ConsultaDados;
...
procedure TFrmConsultaTicket.rectIncluirClick(Sender: TObject);
begin
  inherited;
  ValidarExisteAlgumFuncionario();
end;
